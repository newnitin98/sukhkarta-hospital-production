...
export const metadata: Metadata = {
  title: PAGE_SEO["dr-tanu"].title,
  description: PAGE_SEO["dr-tanu"].description,
  keywords: PAGE_SEO["dr-tanu"].keywords,
  alternates: { canonical: `${SITE_URL}/doctors/${doctor.slug}` },
};

export default function DrTanuPage() {
  return (
    <>
      <SchemaScript
        schema={[
      ...
      <section className="bg-gradient-to-br from-brand-red-700 to-brand-red-800 text-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-start gap-6">
            ...
            <div>
              <div className="text-xs font-semibold text-red-200 uppercase tracking-wider mb-2">
              ...
              <p className="text-red-200 text-sm mt-1">{doctor.subSpeciality}</p>
            </div>
          </div>
        </div>
      </section>
              ...
              <div>
                ...
                <div className="space-y-4">
                  {doctor.education.map((edu) => (
                    <div
                      key={edu.degree}
                      className="flex gap-4 p-4 rounded-xl border border-gray-100 hover:border-red-200 transition-colors"
                    >
                      <GraduationCap className="h-5 w-5 text-brand-red-500 mt-0.5 flex-shrink-0" />
                      <div>
            ...
            <div className="space-y-5">
              <div className="bg-red-50 rounded-2xl p-6 border border-red-100">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div>
                    <span className="text-gray-500">Sub-speciality:</span>{" "}
                    <strong>Interventional Cardiology</strong>
                  </div>
              ...
              <div className="bg-white rounded-2xl shadow-card p-6 space-y-3">
                ...
                <Link
                  ...
                  className="flex items-center justify-center gap-2 w-full border-2 border-brand-red-500 text-brand-red-500 hover:bg-red-50 font-semibold py-2.5 rounded-xl text-sm transition-colors"
                >
                  <Phone className="h-4 w-4" />
                  {doctor.displayPhone}
                </Link>
                <Link
                  href={`tel:${doctor.phoneAlt}`}
                  className="flex items-center justify-center gap-2 w-full border border-red-200 text-red-600 hover:bg-red-50 font-medium py-2.5 rounded-xl text-sm transition-colors"
                >
                  <Phone className="h-4 w-4" />
                  {doctor.displayPhoneAlt}
                </Link>
                <Link
                  href={buildWhatsAppUrl(doctor.phone, HOSPITAL.ctaMessages.cardiology)}
                  target="_blank"
                ...
                <Link
                  ...
                  className="flex items-center justify-center gap-2 w-full border border-gray-200 text-gray-600 hover:text-brand-red-500 py-2.5 rounded-xl text-sm transition-colors"
                >
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-2xl font-bold text-gray-900 font-display mb-6 text-center">
            Request Appointment with Dr. Tanu
          </h2>
          <AppointmentForm />
        </div>
      </section>
